    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.low2 = (
            ...
            if self.n > 1
            else make_low_layer(3, next_dim, next_dim, next_mod, layer=layer, **kwargs)
        )
...
class kp(nn.Module):
    def __init__(
        ...
        self.pre = (
            nn.Sequential(
                convolution(7, 3, 128, stride=2), residual(3, 128, 256, stride=2)
            )
            if pre is None
            else pre
        )
        ...
        self.tl_cnvs = nn.ModuleList([make_tl_layer(cnv_dim) for _ in range(nstack)])
        self.br_cnvs = nn.ModuleList([make_br_layer(cnv_dim) for _ in range(nstack)])
...
class kp_pure_line_cls(nn.Module):
    ...
    def _group_features(self, features, weight):
        ...
        weighted_features = weighted_features.view(
            weighted_features.size(0), -1, 8 * weighted_features.size(2)
        )
    ...
    def _train(self, *xs):
        ...
        layers = zip(self.kps, self.cnvs, self.cls)
        for ind, layer in enumerate(layers):
            kp_, cnv_ = layer[0:2]
            cls_ = layer[2]
            kp = kp_(inter)
...
class kp_pure_box(nn.Module):
    def __init__(
        ...
        super(kp_pure_box, self).__init__()
        print("use kp pure box")
        self.nstack = nstack
        self._decode = _decode_pure
        self.if_dcn = if_dcn
        curr_dim = dims[0]
    ...
    def _train(self, *xs):
        ...
        for ind, layer in enumerate(layers):
            ...
            top_heat, q1_heat, q2_heat, q3_heat, bottom_heat = (
                tl_heat_(tl_cnv),
                br_heat_(tl_cnv),
                tl_heat_(br_cnv),
                br_heat_(br_cnv),
            ...
            top_regr, q1_regr, q2_regr, q3_regr, bottom_regr = (
                tl_regr_(tl_cnv),
                br_regr_(tl_cnv),
                tl_regr_(br_cnv),
                br_regr_(br_cnv),
            ...
            top_regr = _tranpose_and_gather_feat(top_regr, top_inds)
            q1_regr = _tranpose_and_gather_feat(q1_regr, q1_inds)
            q2_regr = _tranpose_and_gather_feat(q2_regr, q2_inds)
            q3_regr = _tranpose_and_gather_feat(q3_regr, q3_inds)
            bottom_regr = _tranpose_and_gather_feat(bottom_regr, bottom_inds)
    ...
    def _test(self, *xs, **kwargs):
        ...
        return self._decode(*outs[-10:], **kwargs), 0, 0
    ...
    def _train(self, *xs):
        ...
        for ind, layer in enumerate(layers):
            ...
            for g_id in range(10):
                key_tag_grouped.append(
                    torch.unsqueeze(
                        _tranpose_and_gather_feat(
                            key_tag_ori, key_inds_grouped[:, g_id, :]
                        ),
                        1,
                    )
                )
...
class AELossLineClsFocal(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        for ps_pre, ng_pre in zip(ps_predictions, ng_predictions):
            ...
            if ps_mask.sum() > 0:
                cls_loss += (
                    torch.pow(1 - ps_pre_n[ps_mask][:, 0], self.lamdb)
                    * self.cls_loss(ps_pre[ps_mask], ps_ind[ps_mask])
                    / 2
                ).mean()
            if ng_mask.sum() > 0:
                cls_loss += (
                    torch.pow(1 - ng_pre_n[ng_mask][:, 1], self.lamdb)
                    * self.cls_loss(ng_pre[ng_mask], ng_ind[ng_mask])
                    / 2
                ).mean()
...
class AELossLine(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        focal_loss += self.focal_loss(
            hybrid_heats, gt_hybrid_heat, self.lamda, self.lamdb
        )
...
class AELossPurePie(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        focal_loss += self.focal_loss(
            center_heats, gt_center_heat, self.lamda, self.lamdb
        )
        focal_loss += self.focal_loss(key_heats, gt_key_heat, self.lamda, self.lamdb)

        regr_loss = 0
        for center_regr, key_regr_tl, key_regr_br in zip(
            center_regrs, key_regrs_tl, key_regrs_br
        ):
...
class AELossBox(nn.Module):
    def __init__(self, lamda, lamdb, regr_weight=1, focal_loss=_neg_loss):
        super(AELossBox, self).__init__()
    ...
    def forward(self, outs, targets):
        ...
        top_heats = outs[0::stride]
        q1_heats = outs[1::stride]
        q2_heats = outs[2::stride]
        q3_heats = outs[3::stride]
        bottom_heats = outs[4::stride]

        top_regrs = outs[5::stride]
        q1_regrs = outs[6::stride]
        q2_regrs = outs[7::stride]
        q3_regrs = outs[8::stride]
        bottom_regrs = outs[9::stride]

        gt_top_heats = targets[0]
        gt_q1_heats = targets[1]
        gt_q2_heats = targets[2]
        gt_q3_heats = targets[3]
        gt_bottom_heats = targets[4]

        gt_mask = targets[5].to(torch.bool)

        gt_top_regrs = targets[6]
        gt_q1_regrs = targets[7]
        gt_q2_regrs = targets[8]
        gt_q3_regrs = targets[9]
        gt_bottom_regrs = targets[10]

        # focal loss
        focal_loss = 0

        tl_heats = [_sigmoid(t) for t in top_heats]
        q1_heats = [_sigmoid(q) for q in q1_heats]
        q2_heats = [_sigmoid(q) for q in q2_heats]
        q3_heats = [_sigmoid(q) for q in q3_heats]
        bottom_heats = [_sigmoid(b) for b in bottom_heats]

        focal_loss += self.focal_loss(tl_heats, gt_top_heats, self.lamda, self.lamdb)
        focal_loss += self.focal_loss(q1_heats, gt_q1_heats, self.lamda, self.lamdb)
        focal_loss += self.focal_loss(q2_heats, gt_q2_heats, self.lamda, self.lamdb)
        focal_loss += self.focal_loss(q3_heats, gt_q3_heats, self.lamda, self.lamdb)
        focal_loss += self.focal_loss(
            bottom_heats, gt_bottom_heats, self.lamda, self.lamdb
        )

        regr_loss = 0
        for top_regr, q1_regr, q2_regr, q3_regr, bot_regr in zip(
            top_regrs, q1_regrs, q2_regrs, q3_regrs, bottom_regrs
        ):
            regr_loss += self.regr_loss(top_regr, gt_top_regrs, gt_mask)
            regr_loss += self.regr_loss(q1_regr, gt_q1_regrs, gt_mask)
            regr_loss += self.regr_loss(q2_regr, gt_q2_regrs, gt_mask)
            regr_loss += self.regr_loss(q3_regr, gt_q3_regrs, gt_mask)
            regr_loss += self.regr_loss(bot_regr, gt_bottom_regrs, gt_mask)
        regr_loss = self.regr_weight * regr_loss

        loss = (focal_loss + regr_loss) / len(top_heats)
        return loss.unsqueeze(0)
