    ...
    def _load_data(self):
        ...
        if not os.path.exists(self._cache_file):
            ...
            def persistent_load(persid):
                # Xử lý persistent ID hoặc thay thế bằng một giá trị mặc định nếu cần
                print(f"Encountered persistent ID: {persid}")
                return ""

            with open(self._cache_file, "rb") as f:
                unpickler = pickle.Unpickler(f)
                unpickler.persistent_load = persistent_load
                self._detections, self._image_ids = unpickler.load()
