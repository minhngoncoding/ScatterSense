...
MIN_NUM_POINTS, MAX_NUM_POINTS = 1, 200
MAX_NUM_MARKERS_TYPES = 5
...
def generate_cluster_points(
    num_points, num_clusters=3, spread=1.0, order_of_magnitude=1
):
    # Define cluster centers
    cluster_centers = [
        (
            np.random.uniform(0, 10**order_of_magnitude),
            np.random.uniform(0, 10**order_of_magnitude),
        )
...
def generate_data_points(num_points, dist_type="uniform", order_of_magnitude=1):
    distributions = {
        ...
        "linear": lambda: (
            np.linspace(0, 10**order_of_magnitude, num_points),
            np.linspace(0, 10**order_of_magnitude, num_points)
            + np.random.normal(0, (10 ** (order_of_magnitude - 1)), num_points),
        ),
        "quadratic": lambda: (
            np.linspace(0, 10**order_of_magnitude, num_points),
            np.linspace(0, 10**order_of_magnitude, num_points) ** 2
            / (10 ** (2 * order_of_magnitude))
            + np.random.normal(0, 0.1, num_points),
        ),
        "sinusoidal": lambda: (
            np.linspace(0, 4 * np.pi, num_points),
            np.sin(np.linspace(0, 4 * np.pi, num_points)) * (10**order_of_magnitude)
            + np.random.normal(0, 0.1, num_points),
        ),
        "exponential": lambda: (
            np.linspace(0, 10**order_of_magnitude, num_points),
        ...
        "logarithmic": lambda: (
            np.linspace(0.1, 10**order_of_magnitude, num_points),
            np.log(np.linspace(0.1, 10**order_of_magnitude, num_points))
            * (10**order_of_magnitude)
            + np.random.normal(0, 0.1, num_points),
        ),
        "cubic": lambda: (
            np.linspace(0, 10**order_of_magnitude, num_points),
            (np.linspace(0, 10**order_of_magnitude, num_points) ** 3)
            / (10 ** (3 * order_of_magnitude))
            + np.random.normal(0, 0.1, num_points),
        ),
        "circular": lambda: (
            np.cos(np.linspace(0, 2 * np.pi, num_points)) * 10**order_of_magnitude
            + np.random.normal(0, 0.1, num_points),
            np.sin(np.linspace(0, 2 * np.pi, num_points)) * 10**order_of_magnitude
            + np.random.normal(0, 0.1, num_points),
        ),
        "elliptical": lambda: (
            np.cos(np.linspace(0, 2 * np.pi, num_points)) * 10**order_of_magnitude
            + np.random.normal(0, 0.1, num_points),
            np.sin(np.linspace(0, 2 * np.pi, num_points)) * (10**order_of_magnitude / 2)
            + np.random.normal(0, 0.1, num_points),
        ),
        "spiral": lambda: (
            np.linspace(0, 2 * np.pi, num_points)
            * np.cos(np.linspace(0, 2 * np.pi, num_points))
            * 10**order_of_magnitude,
            np.linspace(0, 2 * np.pi, num_points)
            * np.sin(np.linspace(0, 2 * np.pi, num_points))
            * 10**order_of_magnitude,
        ),
        "diamond": lambda: (
        ...
        "threshold_scatter": lambda: (
            np.random.normal(0, 5, num_points),  # Scatter with quadrants
            np.random.normal(0, 5, num_points),
        ),
        "dense_vertical": lambda: (
            np.random.uniform(-5, 5, num_points),  # Dense vertical scatter
            np.random.normal(0, 2, num_points),
        ),
        "cluster": lambda: generate_cluster_points(
            num_points,
...
def map_data_to_image_coordinates(x, y, ax, fig, dpi, radius_pixels):
    ...
    for data_x, data_y, radius in zip(x, y, radius_pixels):
        ...
        pixel_y = (
            axes_y0
            + axes_height
            - (data_y - ylim[0]) / (ylim[1] - ylim[0]) * axes_height
        )
...
def generate_scatter_plot_with_metadata(image_name, image_id):
    ...
    dist_type = (
        random.choice(DISTRIBUTION_TYPES)
    ...
    if use_mixed_markers:
        ...
        # Shuffle all indices
        indices = (
            np.random.permutation(num_points)
    ...
    if np.random.rand() <= ADDING_GRID_PROB:
        ...
        ax.grid(
            True, color="black", linestyle=major_linestyle, linewidth=major_linewidth
        )
        if np.random.rand() <= ADDING_GRID_PROB:
            ax.minorticks_on()
    ...
    for data in plot_data:
        ...
        bbox, areas = map_data_to_image_coordinates(
            x[group], y[group], ax, fig, dpi, radius_in_pixels
        )
...
if __name__ == "__main__":
    ...
    metadata_list = {"images": [], "annotations": [], "categories": []}
    ...
    output_dir = "data/scatterdata/scatter/images/test"
    ...
    for i in tqdm(range(NUM_IMAGES)):
        ...
        image_metadata, plot_metadata = generate_scatter_plot_with_metadata(
            image_name, image_id
        )
        image_id += 1
    ...
    print(
        f"Generated {NUM_IMAGES} plots with pixel-based metadata saved to '{metadata_file}'."
    )
