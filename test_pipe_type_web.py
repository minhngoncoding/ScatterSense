...
def Pre_load_nets(type, id_cuda):
    ...
    if type == 2:
        db_pie, nnet_pie = load_net(
            50000,
            "CornerNetPurePie",
            "data/piedata(1008)",
            "data/piedata(1008)/cache",
    ...
    if type == 1:
        ...
        db_line_cls, nnet_line_cls = load_net(
            20000,
            "CornerNetLineClsReal",
            "data/linedata(1028)",
            "data/linedata(1028)/cache",
            "data/linedata(1028)/result",
            id_cuda,
...
def ocr_result(image_path):
    ...
    while "recognitionResults" not in analysis.keys():
        time.sleep(3)
        binary_content = requests.get(
            op_location, headers=headers, params=params
        ).content
...
def test(
    ...
    with torch.no_grad():
        ...
        if data_type == 2:
            ...
            results = methods["Pie"][2](
                image, methods["Pie"][0], methods["Pie"][1], debug=False
            )
            cens = results[0]
            keys = results[1]
        ...
        if data_type == 1:
            ...
            results = methods["Line"][2](
                image, methods["Line"][0], methods["Line"][1], debug=False, cuda_id=1
            )
            keys = results[0]
            hybrids = results[1]
            image_painted, quiry, keys, hybrids = GroupQuiryRaw(
                image_draw, keys, hybrids
            )
            results = methods["LineCls"][2](
                image,
                methods["LineCls"][0],
                quiry,
                methods["LineCls"][1],
