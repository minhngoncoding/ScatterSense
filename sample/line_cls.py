...
def random_crop_line(image, ps_detections, ng_detections, random_scales, view_size, border=64):
    ...
    # print(ps_detections.shape)
    # print(ng_detections.shape)
    ...
    print("ps shape: ", ps_cropped_detections.shape)
    ps_cropped_detections[:, 0:ps_cropped_detections.shape[1]:2] -= x0
    ps_cropped_detections[:, 1:ps_cropped_detections.shape[1]:2] -= y0
    ps_cropped_detections[:, 0:ps_cropped_detections.shape[1]:2] += cropped_ctx - left_w
    ps_cropped_detections[:, 1:ps_cropped_detections.shape[1]:2] += cropped_cty - top_h


    ng_cropped_detections = ng_detections.copy()
    print("ng shape: ", ng_cropped_detections.shape)
    print(ng_detections)
    print(ps_detections)
...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    for b_ind in range(batch_size):
        ...
        while not flag:
            ...
            #print(temp)
            print("ps_detections: {}".format(ps_detections))
