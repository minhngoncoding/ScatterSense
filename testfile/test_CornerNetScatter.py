...
def kp_decode(nnet, images, K, ae_threshold=0.5, kernel=3):
    with torch.no_grad():
        detections_tl_detections_br, time_backbone, time_psn = nnet.test(
...
def kp_detection(image, db, nnet, debug=False, decode_func=kp_decode, cuda_id=0):
    ...
    nms_algorithm = {"nms": 0, "linear_soft_nms": 1, "exp_soft_nms": 2}[
    ...
    if True:
        ...
        dets_key, dets_hybrid, time_backbone, time_psn, flag = decode_func(
        ...
        for j in range(categories):
            ...
            top_points_hybrid[j + 1] = detections_point_hybrid[keep_inds_p].astype(
                np.float32
            )
            # print(top_points[image_id][j + 1][0])

        scores = np.hstack([top_points_key[j][:, 0] for j in range(1, categories + 1)])
        ...
        scores = np.hstack(
            [top_points_hybrid[j][:, 0] for j in range(1, categories + 1)]
        )
        if len(scores) > max_per_image:
            kth = len(scores) - max_per_image
            thresh = np.partition(scores, kth)[kth]
...
def testing(db, nnet, result_dir, debug=False, cuda_id=0):
    return globals()[system_configs.sampling_function](
        db, nnet, result_dir, debug=debug, cuda_id=cuda_id
    )
